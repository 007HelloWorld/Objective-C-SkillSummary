...
- (void)configureTopView {
    ...
    [leftBtn addTarget:self action:@selector(clickTopLeftBtn:) forControlEvents:UIControlEventTouchUpInside];
    [leftBtn setImage:[UIImage imageNamed:@"nav_back_w.png"] forState:UIControlStateNormal];
...
- (void)initHistory {
    if (!self.historyRecords) {
        self.historyRecords = [[NSMutableArray alloc]init];
    }
...
#pragma mark - Event_Click

- (void)clickTopLeftBtn:(UIButton *)sender {
    //返回
    [self.navigationController popViewControllerAnimated:YES];
}
